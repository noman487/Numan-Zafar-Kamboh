...
import { Download, FileText, ImageIcon, ChevronsRight, AlertTriangle, X, Archive, Image as ImageIconLucide, Bookmark, UploadCloud, Trash2, ListOrdered } from 'lucide-react';
...
export default function App() {
  ...
  const [niche, setNiche] = useState('Storytelling');
  const [imageStyle, setImageStyle] = useState('photorealistic, ultra HD,realistic, 4k ultra-HD, DSLR camera, natural, filtered, 4k');
  const [negativePrompts, setNegativePrompts] = useState('text, watermark, blurry, noise, signature, ugly, deformed');
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
        ...
        if (promptSource === 'script') {
            ...
            const { prompts: generated, requestPrompt } = await generatePrompts(script, effectiveImageStyle, niche, numPrompts);
            prompts = generated;
            ...
            const promptRegex = /^\s*(\d+)\s*[:=\-.]\s*(.+)$/;
            const parsedPrompts: { index: number; text: string }[] = [];

            customPrompts.trim().split('\n').forEach(line => {
                const match = line.trim().match(promptRegex);
                if (match) {
                    parsedPrompts.push({
            ...
            if (parsedPrompts.length === 0) {
                setError('No valid numbered prompts found. Use a format like "1. Your prompt".');
                setAppState(AppState.IDLE);
                return;
            }
        ...
        for (let i = 0; i < prompts.length; i++) {
            setProgressMessage(`Generating image ${i + 1} of ${prompts.length} (${numVariations} variations)...`);

            let finalPrompt = prompts[i];
            if (promptSource === 'custom' && effectiveImageStyle.trim()) {
                finalPrompt = `${prompts[i]}, ${effectiveImageStyle}`;
            }

            let results: { base64: string | null; error: string | null; }[] = [];
            let engine: GeneratedImage['engine'] = 'google';

            results = await generateImage(finalPrompt, aspectRatio, negativePrompts, numVariations);
            
            if (results.some(r => r.error && r.error.includes('safety reasons')) && process.env.STABILITY_API_KEY) {
                setProgressMessage(`Prompt ${i + 1} rejected by Google. Retrying with Stability AI...`);
                engine = 'stability';
                results = await generateImageWithStability(finalPrompt, aspectRatio, negativePrompts, numVariations);
            }
            ...
            imageResults.push(...newImages);
            setGeneratedImages([...imageResults]);
        }
      
        setAppState(AppState.DONE);
        const successfulImages = imageResults.filter(img => !img.error).length;
        if (successfulImages === (prompts.length * numVariations)) {
            setProgressMessage('All images generated successfully!');
        } else {
            setProgressMessage(`Generated ${successfulImages} of ${prompts.length * numVariations} images. Some may have failed.`);
        }
  ...
  }, [script, imageStyle, niche, aspectRatio, promptSource, customPrompts, referenceImageFile, negativePrompts, numVariations, numPrompts]);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200 font-sans p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col gap-6 p-6 bg-gray-800/50 rounded-2xl border border-gray-700 shadow-lg">
                  ...
                  >
                   ...
                   <div>
                      <label htmlFor="niche-input" className="flex items-center text-sm font-semibold mb-2 text-gray-300">
                        ...
                        Niche / Storytelling Topic (Optional)
                      </label>
                      <input
                        id="niche-input"
                        type="text"
                        ...
                        placeholder="e.g., historical drama, sci-fi adventure"
                      ...
                      />
                       <p className="text-xs text-gray-500 mt-1">Provide a storytelling topic to give the AI more context about the script.</p>
                    </div>
                     <div>
                      <label htmlFor="num-prompts-input" className="flex items-center text-sm font-semibold mb-2 text-gray-300">
                          <ListOrdered className="w-4 h-4 mr-2 text-primary-400" />
                          Minimum Prompts to Generate
                      </label>
                      <input
                          id="num-prompts-input"
                          type="number"
                          value={numPrompts}
                          onChange={(e) => setNumPrompts(Math.max(1, Math.min(40, parseInt(e.target.value, 10) || 1)))}
                          min="1"
                          max="40"
                          className="w-full p-3 bg-gray-900 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"
                          disabled={isLoading || !isScriptMode}
                      />
                      <p className="text-xs text-gray-500 mt-1">Specify at least how many prompts to generate from the script (1-40).</p>
                    </div>
                </div>
              ) : (
                <textarea
                  ...
                  placeholder="Use numbers to order your prompts, e.g., '1. A futuristic city' or '2=A robot'."
            ...
            <section>
              ...
              <div className="flex flex-col gap-4 p-4 bg-gray-900/50 border border-gray-700 rounded-lg">
                  ...
                  <div>
                      <label htmlFor="negative-prompt-input" className="text-sm font-semibold text-gray-300 mb-1 block">Negative Prompts (Optional)</label>
                      <p className="text-xs text-gray-500 mb-2">Keywords for concepts to exclude from the image.</p>
                      <input
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                ...
                <div>
                  <label htmlFor="num-variations" className="text-sm font-semibold text-gray-300 mb-2 block">Variations per Prompt</label>
                  <input
                      id="num-variations"
                      type="number"
                      value={numVariations}
                      ...
                      min="1"
                      max="4"
                      className="w-full p-2 bg-gray-900 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors"
                      disabled={isLoading}
                  />
                  <p className="text-xs text-gray-500 mt-1">Number of images to generate for each prompt (1-4).</p>
                </div>
              </div>
            </section>
                 ...
                 {(generatedPrompts.length > 0 || isLoading) && (
                    <div className="flex-shrink-0">
                      ...
                      <div className="h-32 p-3 bg-gray-900 border border-gray-600 rounded-lg overflow-y-auto">
                        ...
                        {generatedPrompts.map((p, i) => <p key={i} className="text-sm text-gray-300 py-1 border-b border-gray-700/50 last:border-b-0">{p}</p>)}
                      </div>
                    </div>
                )}
                        ...
                        {generatedImages.length > 0 ? (
                           <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {generatedImages.map((image, index) => (
                                    <div key={index} className="group relative aspect-w-1 aspect-h-1 bg-gray-800 rounded-lg overflow-hidden">
                                        {image.url ? (
                                            <>
                                                ...
                                                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2 cursor-pointer" onClick={() => openImagePreview(image)}>
                                                    <p className="text-xs text-white truncate">{image.prompt}</p>
                                                     <a
                                                        href={image.url}
                                                        download={`image_${image.engine || 'unknown'}_${index + 1}.jpeg`}
                                            ...
                                            <div className="w-full h-full flex flex-col items-center justify-center p-3 text-center bg-red-900/20 border border-red-800 rounded-lg">
                                                ...
                                                <p className="text-xs text-gray-500 mt-2 line-clamp-2">Prompt: {image.prompt}</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
      ...
      {imagePreviewModalOpen && currentPreviewImage && (
         <Modal title="Image Preview" onClose={() => setImagePreviewModalOpen(false)} maxWidth="max-w-4xl">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-primary-400 mb-2">Prompt</h3>
                <p className="p-3 bg-gray-800 rounded-md text-gray-300 text-sm">{currentPreviewImage.prompt}</p>
                <a
                  href={currentPreviewImage.url!}
                  download={`image_${currentPreviewImage.engine || 'unknown'}_${generatedImages.filter(i => !i.error).findIndex(img => img.url === currentPreviewImage.url) + 1}.jpeg`}
